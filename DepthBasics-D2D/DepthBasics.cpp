...
//    else for (i = 0; i<nArgs; i++) printf("%d: %ws\n", i, szArglist[i]);
    ...
    if (nArgs > 0) application.m_pDepthFilePath = szArglist[1];
...
/// </summary>
void CDepthBasics::Update(HWND hWndApp)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            if (m_pDepthFilePath) {
                SaveDepthToFile(nTime, pBuffer, nWidth, nHeight, sizeof(UINT16) * 8, m_pDepthFilePath);
...
HRESULT CDepthBasics::SaveDepthToFile(INT64 nTime, const UINT16* pBuffer, LONG lWidth, LONG lHeight, WORD wBitsPerPixel, LPCWSTR lpszFilePath)
{
    DWORD dwByteCount = lWidth * lHeight * (wBitsPerPixel / 8);
    ...
    // Write the depth file header
    DEPTHFILEHEADER dfh;
    dfh.nTime = nTime;
    dfh.lWidth = lWidth;
    dfh.lHeight = lHeight;
    dfh.wBitsPerPixel = wBitsPerPixel;

    if (!WriteFile(hFile, &dfh, sizeof(dfh), &dwBytesWritten, NULL))
    {
        CloseHandle(hFile);
        return E_FAIL;
    }
    ...
    if (!WriteFile(hFile, pBuffer, dwByteCount, &dwBytesWritten, NULL))
    {
        CloseHandle(hFile);
        return E_FAIL;
    }
