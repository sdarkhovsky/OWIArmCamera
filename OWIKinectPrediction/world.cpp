...
void c_world::match_observed_parts(std::unique_ptr<c_world_part> observed_part) {
    ...
    if (world_parts.empty()) {
        world_parts.push_back(std::move(observed_part));
        return;
    }
...
void c_world::add_observation( c_point_cloud& point_cloud>, double time) {
    ...
    match_observed_parts(observed_parts); 
}
